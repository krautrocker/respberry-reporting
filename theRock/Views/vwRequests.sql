CREATE                     VIEW [theRock].[vwRequests] AS
SELECT
	CAST(ROW_NUMBER() OVER(ORDER BY (SELECT NULL)) AS INT) AS requestId,
	...
	JSON_VALUE(data.value, '$.backfromStatus') AS backformStatus,
	JSON_VALUE(data.value, '$.destinationFacilityCode') AS destinationFacilityCode,
	JSON_VALUE(data.value, '$.requestStatus') AS requestStatus,
	JSON_VALUE(data.value, '$.requestCreatedAt') AS requestCreatedAt,
	JSON_VALUE(data.value, '$.referralStatus') AS referralStatus,
	JSON_VALUE(data.value, '$.requestUpdatedAt') AS requestUpdatedAt,
